...
def chmap(channels):
    ch_list = [4, 5, 19, 34, 51, 77, 95, 127, 152, 342, 399, 401, 422, 434, 442, 469, 484, 487, 499, 538, 555, 588, 637, 664, 676, 683, 686, 750, 837, 879, 905, 934, 949, 964]
    mapped = []
    for c in channels:
...
def parse_args():
   ...
   parser.add_argument('--set', type=str, default='trainext', choices=['trainext'])
...
if __name__ == '__main__':
    ...
    if opt.n_swirs > 0:
        ...
        elif opt.sel == 'uniform':
            assert(int(np.sqrt(opt.n_swirs))**2 == opt.n_swirs)
            if opt.n_swirs == 1:
                swir_channels.append(480)
            else:
                a = int(np.sqrt(opt.n_swirs))
                for i in range(0, a):
                    for j in range(0, a):
                        x = int(np.floor(31*(2*i+1)/2/a))
                        y = int(np.floor(31*(2*j+1)/2/a))
                        swir_channels.append(x * 31 + y)
        else:
    ...
    for lid, light in enumerate(lights):
        for i in range(n_scenes_per_light):
            idx = str(i).zfill(2)
            print()
            print('scene', light, idx)

            print(file=log_file)
            print('scene', light, idx, file=log_file)

            scene_path = test_path / light / 'scene'
            bgscene_path = test_path / light / 'bgscene'
            label_path = test_path / light / 'label'

            scene = np.load(scene_path / (idx + '_scene.npz'))
            scene = np.concatenate((scene['rgbn'][:, :, rgbn_channels], scene['swir'][:, :, chmap(swir_channels)]), axis=2)
            ...
            if opt.bg == 'inpaint':
                ...
                bgscene = np.concatenate((bgscene['rgbn'][:, :, rgbn_channels], bgscene['swir'][:, :, chmap(swir_channels)]), axis=2)
            else:
                assert(False)
