...
def test(opt, test_loader, testext_loader, net, split):
    ...
    for iteration, batch in enumerate(test_loader):
        ...
        print('test', str(iteration * opt.batch_size + i).zfill(2), time.time() - start_time, 'seconds')
    ...
    for iteration, batch in enumerate(testext_loader):
        ...
        for i in range(opt.batch_size):
            ...
            prob_crf = crf(prob_np, im_np, opt.sdims, opt.schan, opt.compat, opt.iters)
            eva_crf.register(label_np, prob_crf)
        print('testext', str(iteration * opt.batch_size + i).zfill(2), time.time() - start_time, 'seconds')
...
if __name__ == '__main__':
    ...
    (Path(opt.out_path) / 'merge').mkdir(parents=True, exist_ok=True)
    ...
    testext_set = RealDataset(opt.real_path, opt.channels, split='testext')
    testext_loader = DataLoader(dataset=testext_set, num_workers=opt.threads, batch_size=opt.batch_size, shuffle=False)

    net = PowderNet(opt.arch, opt.n_channels, opt.n_classes)
    net = net.cuda()
    net.load_state_dict(checkpoint['state_dict'])

    log_file = open(Path(opt.out_path) / 'performance_merge.txt', 'w')
    print(opt, file=log_file)
    msa, miou, miiou, msa_crf, miou_crf, miiou_crf = test(opt, test_loader, testext_loader, net, 'merge')
    print('Merge  Pre-CRF:  MSA: {}   mIoU: {}   miIoU: {}'.format(round(msa * 100, 1), round(miou * 100, 1), round(miiou * 100, 1)), file=log_file)
    print('Merge  Post-CRF: MSA: {}   mIoU: {}   miIoU: {}'.format(round(msa_crf * 100, 1), round(miou_crf * 100, 1), round(miiou_crf * 100, 1)), file=log_file)
