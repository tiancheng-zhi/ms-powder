...
class Block(nn.Module):
    def __init__(self, inplanes, planes, reps, stride=1, dilation=1, start_with_relu=True, grow_first=True):
        ...
        if planes != inplanes or stride != 1:
            ...
            self.skipbn = nn.GroupNorm(8, planes)
        else:
            self.skip = None
        ...
        if grow_first:
            ...
            rep.append(nn.GroupNorm(8, planes))
            filters = planes
...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, pretrained=False, _print=True):
        ...
        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                             ...
                                             nn.GroupNorm(32, 256),#nn.BatchNorm2d(256),
                                             nn.ReLU())
    ...
    def forward(self, input):
        ...
        y = self.bn1(y)
        y = self.relu(y)
        y = F.interpolate(y, size=low_level_features.size()[2:], mode='bilinear', align_corners=True)

        z = torch.cat((y, low_level_features), dim=1)
        z = self.last_conv(z)
        z = F.interpolate(z, size=input.size()[2:], mode='bilinear', align_corners=True)
